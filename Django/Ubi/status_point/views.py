...
OPERATORS = {
        ...
        'lte_gte' : lambda count, count2 : StatusPoint.objects.filter(indicator_count__lte=count, indicator_count__gte=count2).order_by('-name'),
...
class StatusPointView(APIView):
    def get(self, request, format=None):
        ...
        indicator_count_2 = request.GET.get('indicator_count_2','')
        ...
        if indicator_count and operator:
            ...
            if not filter:
                return Response({'message':'Incorrect Operator (lte, gte, lte_gte, e)'}, status=status.HTTP_400_BAD_REQUEST)

            if indicator_count_2:
                points = filter(indicator_count, indicator_count_2)
            else:
